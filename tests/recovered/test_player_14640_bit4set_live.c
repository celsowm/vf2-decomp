/* ROM-backed differential fixture for the fa_rob bit-4-set neutral arm
 * 0x14640 (v0407).
 *
 * The 0x14640 arm is the fa_rob fighter-state helper reached when the
 * fighter g7 has +0x198 == 0, +0x654 == 0, +0x197 not 27/28 and bit 4 of
 * (g7) SET (so the `0x146b0 bbc 4, r15` is not taken), with r3 (= +0x197)
 * != 13 (so the `0x146b8 cmpobe 13, r3` is also not taken).  It clears
 * +0x194(g7) and leaves r15 = 0 and r3 = +0x197.  No walker.
 *
 * This fixture restores out/park-1442c.vf2snap, jumps to the 0x14640 entry
 * with the bit-4-set shape (fighter0 +0x197 = 0, bit 4 of (g7) set), then
 * runs the reference interpreter and the native helper to ip == 0x146c4 and
 ...
 *     --set-u8 0x510B17=0 --set-u32 0x510980=0x2014 --until 0x146c4
 *     -> 12 steps, +0 call / +0 return.
 */
...
#define BIT4_ENTRY UINT32_C(0x00014640)
#define BIT4_RETURN UINT32_C(0x000146c4)

#define REF_TOTAL UINT64_C(12)
...
static void test_unit_invalid_arguments(void)
{
    ...
    CHECK(
        vf2_hybrid_player_14640_bit4set_execute_for_test(NULL, &cpu) != VF2_OK
    );
    CHECK(
        vf2_hybrid_player_14640_bit4set_execute_for_test(&machine, NULL) !=
        VF2_OK
    );
}

/* ROM-independent fail-closed checks for the bit-4-set neutral arm.  The
 * arm must refuse any non-matching shape (wrong ip, +0x198 != 0,
 * +0x654 != 0, +0x197 == 27/28/13, bit 4 of (g7) clear, zero fighter base,
 * no pushed frame) before touching memory. */
static void test_unit_fail_closed(void)
{
    ...
    CHECK(
        vf2_hybrid_player_14640_bit4set_execute_for_test(&machine, &cpu) !=
        VF2_OK
    );
...
)
{
    ...
    /* Force the measured bit-4-set shape on both machines: +0x197 = 0 (not
     * 27/28/13) and bit 4 of (g7) set.  +0x198/+0x654 are already 0. */
    reference_cpu.ip = BIT4_ENTRY;
    native_cpu.ip = BIT4_ENTRY;
    ...
    /* Reference: step the 0x14640 bit-4-set arm to its 0x146c4 ret
     * instruction (12 steps / +0 call / +0 return). */
    steps = 0u;
    while (reference_cpu.ip != BIT4_RETURN && steps < 1024u) {
        reference_status =
    ...
    CHECK(reference_cpu.ip == BIT4_RETURN);
    ...
    /* Native: the 0x14640 bit-4-set neutral arm. */
    native_status = vf2_hybrid_player_14640_bit4set_execute_for_test(
        &native_machine, &native_cpu);
    CHECK(native_status == VF2_OK);
    CHECK(native_cpu.ip == BIT4_RETURN);
    native_instructions =
    ...
    printf(
        "player-14640-bit4set-live ref=%llu native=%llu calls=%llu/%llu rets=%llu/%llu\n",
    ...
    if (compare_status != VF2_OK || !diff.equal) {
        fprintf(
            stderr,
            "player-14640-bit4set-live ref=%d native=%d compare=%d "
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        if (failures != 0) {
            fprintf(
                stderr, "%d player-14640-bit4set-live unit test(s) failed\n",
                failures);
            return EXIT_FAILURE;
        }
        puts("player-14640-bit4set-live ROM-independent tests passed");
        return EXIT_SUCCESS;
    }
    ...
    if (failures != 0) {
        fprintf(
            stderr,
            "%d player-14640-bit4set-live differential test(s) failed\n",
            failures
        );
        return EXIT_FAILURE;
    }
    puts("player-14640-bit4set-live differential tests passed");
    return EXIT_SUCCESS;
}
