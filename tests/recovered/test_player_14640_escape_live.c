/* ROM-backed differential fixture for the fa_rob escape arm 0x14640 (v0408).
 *
 * The 0x14640 arm is the fa_rob fighter-state helper reached when the fighter
 * g7 has +0x198 != 0 (so the `0x14644 cmpobne 0, r3` jumps directly to
 * 0x146dc).  It stores r3 (= the +0x198 value) to +0x194(g7), clears
 * +0x654(g7) and leaves r15 = 0 and r3 = +0x198.  No walker.
 *
 * This fixture restores out/park-1442c.vf2snap, jumps to the 0x14640 entry
 * with +0x198 set non-zero, then runs the reference interpreter and the native
 * helper to ip == 0x146e8 and asserts exact step/call/ret lockstep plus full
 * live-state equality (registers/CC/AC/frames/Work-RAM).
 *
 ...
 *     --set-u32 0x510B18=0x7B --until 0x146e8
 *     -> 5 steps, +0 call / +0 return.
 */
...
#define ESCAPE_ENTRY UINT32_C(0x00014640)
#define ESCAPE_RETURN UINT32_C(0x000146e8)

#define REF_TOTAL UINT64_C(5)
...
static void test_unit_invalid_arguments(void)
{
    ...
    CHECK(
        vf2_hybrid_player_14640_escape_execute_for_test(NULL, &cpu) != VF2_OK
    );
    CHECK(
        vf2_hybrid_player_14640_escape_execute_for_test(&machine, NULL) !=
        VF2_OK
    );
}

/* ROM-independent fail-closed checks for the escape arm.  The arm must refuse
 * any non-matching shape (wrong ip, +0x198 == 0, zero fighter base, no pushed
 * frame) before touching memory. */
static void test_unit_fail_closed(void)
{
    ...
    CHECK(
        vf2_hybrid_player_14640_escape_execute_for_test(&machine, &cpu) !=
        VF2_OK
    );
...
)
{
    ...
    /* Force the measured escape shape on both machines: +0x198 != 0. */
    reference_cpu.ip = ESCAPE_ENTRY;
    native_cpu.ip = ESCAPE_ENTRY;
    write_u32(&reference_machine, fighter0 + UINT32_C(0x198), 0x0000007Bu);
    write_u32(&native_machine, fighter0 + UINT32_C(0x198), 0x0000007Bu);
    ...
    /* Reference: step the 0x14640 escape arm to its 0x146e8 ret instruction
     * (5 steps / +0 call / +0 return). */
    steps = 0u;
    while (reference_cpu.ip != ESCAPE_RETURN && steps < 1024u) {
        reference_status =
    ...
    CHECK(reference_cpu.ip == ESCAPE_RETURN);
    ...
    /* Native: the 0x14640 escape arm. */
    native_status = vf2_hybrid_player_14640_escape_execute_for_test(
        &native_machine, &native_cpu);
    CHECK(native_status == VF2_OK);
    CHECK(native_cpu.ip == ESCAPE_RETURN);
    native_instructions =
    ...
    printf(
        "player-14640-escape-live ref=%llu native=%llu calls=%llu/%llu rets=%llu/%llu\n",
    ...
    if (compare_status != VF2_OK || !diff.equal) {
        fprintf(
            stderr,
            "player-14640-escape-live ref=%d native=%d compare=%d "
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        if (failures != 0) {
            fprintf(
                stderr, "%d player-14640-escape-live unit test(s) failed\n",
                failures);
            return EXIT_FAILURE;
        }
        puts("player-14640-escape-live ROM-independent tests passed");
        return EXIT_SUCCESS;
    }
    ...
    if (failures != 0) {
        fprintf(
            stderr,
            "%d player-14640-escape-live differential test(s) failed\n",
            failures
        );
        return EXIT_FAILURE;
    }
    puts("player-14640-escape-live differential tests passed");
    return EXIT_SUCCESS;
}
