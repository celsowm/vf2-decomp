/* ROM-backed differential fixture for the fa_rob compare-prefix arm 0x14640
 * (v0409).
 *
 ...
 * with s16(+0x1aa) > s16(+0x62a) (so the `0x14658 cmpobe r13, r14` is not
 * taken to 0x146dc), +0x197 not 27/28/13 and bit 4 of (g7) SET (so the
 * neutral bit-4-set tail 0x146bc..0x146c4 runs).  It clears +0x194(g7) and
 * leaves r15 = 0, r3 = +0x197, r13 = s16(+0x1aa), r14 = s16(+0x62a).  No
 * walker.
 *
 * This fixture restores out/park-1442c.vf2snap, jumps to the 0x14640 entry
 * with +0x654 set non-zero and bit 4 of (g7) set, then runs the reference
 ...
 *     --set-u32 0x510FD4=0x1 --set-u32 0x510980=0x2014 --until 0x146c4
 *     -> 15 steps, +0 call / +0 return.
 */
...
#define COMPARE_ENTRY UINT32_C(0x00014640)
#define COMPARE_RETURN UINT32_C(0x000146c4)

#define REF_TOTAL UINT64_C(15)
...
static void test_unit_invalid_arguments(void)
{
    ...
    CHECK(
        vf2_hybrid_player_14640_compare_execute_for_test(NULL, &cpu) != VF2_OK
    );
    CHECK(
        vf2_hybrid_player_14640_compare_execute_for_test(&machine, NULL) !=
        VF2_OK
    );
}

/* ROM-independent fail-closed checks for the compare-prefix arm.  The arm must
 * refuse any non-matching shape (wrong ip, zero fighter base, no pushed frame)
...
static void test_unit_fail_closed(void)
{
    ...
    CHECK(
        vf2_hybrid_player_14640_compare_execute_for_test(&machine, &cpu) !=
        VF2_OK
    );
...
)
{
    ...
    /* Force the measured compare-prefix shape on both machines: +0x654 != 0,
     * bit 4 of (g7) set, +0x197 = 0 (not 27/28/13).  +0x198 is already 0 and
     * s16(+0x1aa) = 1 > s16(+0x62a) = 0 in the park. */
    reference_cpu.ip = COMPARE_ENTRY;
    native_cpu.ip = COMPARE_ENTRY;
    write_u32(&reference_machine, fighter0 + UINT32_C(0x654), 0x00000001u);
    write_u32(&native_machine, fighter0 + UINT32_C(0x654), 0x00000001u);
    write_u8(&reference_machine, fighter0 + UINT32_C(0x197), 0u);
    write_u8(&native_machine, fighter0 + UINT32_C(0x197), 0u);
    ...
    /* Reference: step the 0x14640 compare-prefix arm to its 0x146c4 ret
     * instruction (15 steps / +0 call / +0 return). */
    steps = 0u;
    while (reference_cpu.ip != COMPARE_RETURN && steps < 1024u) {
        reference_status =
    ...
    CHECK(reference_cpu.ip == COMPARE_RETURN);
    ...
    /* Native: the 0x14640 compare-prefix arm. */
    native_status = vf2_hybrid_player_14640_compare_execute_for_test(
        &native_machine, &native_cpu);
    CHECK(native_status == VF2_OK);
    CHECK(native_cpu.ip == COMPARE_RETURN);
    native_instructions =
    ...
    printf(
        "player-14640-compare-live ref=%llu native=%llu calls=%llu/%llu rets=%llu/%llu\n",
    ...
    if (compare_status != VF2_OK || !diff.equal) {
        fprintf(
            stderr,
            "player-14640-compare-live ref=%d native=%d compare=%d "
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        if (failures != 0) {
            fprintf(
                stderr, "%d player-14640-compare-live unit test(s) failed\n",
                failures);
            return EXIT_FAILURE;
        }
        puts("player-14640-compare-live ROM-independent tests passed");
        return EXIT_SUCCESS;
    }
    ...
    if (failures != 0) {
        fprintf(
            stderr,
            "%d player-14640-compare-live differential test(s) failed\n",
            failures
        );
        return EXIT_FAILURE;
    }
    puts("player-14640-compare-live differential tests passed");
    return EXIT_SUCCESS;
}
