...
Can the no-bit-8 state-8 composition `0x00000036` (bits 1+2+4+5+6)
use the existing native mixed-low child at threshold 3 without widening
unmeasured thresholds or distributions?
...
Before the admission, all 12 cases reached the native corridor but differed
...
`hybrid_execute_game_info_bit31_native` admits `0x36` only for the measured
matrix distribution and threshold `<= 3`. The existing mixed-low accounting
