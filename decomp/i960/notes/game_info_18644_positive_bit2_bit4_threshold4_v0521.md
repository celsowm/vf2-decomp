# `fa_game_info` positive mixed masks through threshold 4 (v0521)
...
threshold `4`. Both fighter state bytes remain `8`, and the complete matrix
covers the three measured fighter-flag distributions, countdown `0` and `1`,
and mode bit 6 clear and set:

```text
5 masks × 3 distributions × 2 countdowns × 2 mode values = 60 cases
```

All 60 cases match the reference at the full dispatcher boundary, including
...
distributions, state `8` on both fighters and thresholds `0..4`. Threshold
`5+`, the adjacent `0x18`, other flag distributions and unmeasured positive
...
python decomp/i960/tools/validate_game_info_full_dispatch.py \
  build/Debug/vf2i960.exe roms/vf2 --mask 0x14 --thresholds 4 \
