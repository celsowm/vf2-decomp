# `fa_game_info` positive state-8 bit-1/bit-6 medium families v0481

The calibrated `0x164ac` boundary was swept through both `0x18644` child
calls and the scheduler return for the three measured masks:
...
The `0x8142` and `0x10142` families share the measured positive medium-bit
admission predicate. Their child-call instruction counts require mask-local
corrections at the two dispatcher return sites; no call, return, condition,
memory or device-visible state differences were observed.
