# `fa_game_info` positive state-8 mask `0x0a` (v0526)

The measured positive state-8 composition `0x0000000a` (bits 1+3) is now
...
mutable Model 2A memory. The measured native path requires a uniform
two-instruction correction for both countdown values.

Admission remains limited to the nine measured low masks, the three measured
...
compositions remain `VF2_ERROR_UNSUPPORTED`. Neighboring masks `0x0c` and
`0x0e` were measured as different paths and remain fail-closed.

## Reproduction

```text
python decomp/i960/tools/validate_game_info_full_dispatch.py \
  build/Debug/vf2i960.exe roms/vf2 --mask 0xa --thresholds 0,1,2,3,4,5,6,7,8 \
