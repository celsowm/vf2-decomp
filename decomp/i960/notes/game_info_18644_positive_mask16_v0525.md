# `fa_game_info` positive state-8 mask `0x16` (v0525)

The measured positive state-8 composition `0x00000016` (bits 1+2+4) is
...
Admission remains limited to the eight measured low masks, the three measured
...
compositions remain `VF2_ERROR_UNSUPPORTED`. Neighboring masks `0x0a`,
`0x0c` and `0x0e` were measured as different paths and remain fail-closed.

## Reproduction

```text
python decomp/i960/tools/validate_game_info_full_dispatch.py \
  build/Debug/vf2i960.exe roms/vf2 --mask 0x16 --thresholds 0,1,2,3,4,5,6,7,8 \
