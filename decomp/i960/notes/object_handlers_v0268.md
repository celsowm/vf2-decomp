...
## Negative scout (2026-09-05, recorded so the next slice skips it)

From `scratch-sixth.vf2snap`: live coli entry `0x51498c = 0x221cc`,
flags `0x514980 = 0`, fighters `0x510a40/0x512a40`. Forcing
`0x514980=0x80000000` + `0x51498c=0x221e8` with `--until 0x221e8` over
1M probe steps / 15.3M guest instructions never reaches `0x221e8`
(end `ip=0x10fa0`, call/return counters identical to the unmutated run)
and the entry is never re-armed to `0x221cc` either. The sixth-snapshot
window therefore contains neither the selector2 re-arm nor any sweep
that dispatches index 10. Next attempt should start from an earlier
snapshot (e.g. `native-fifth-dispatch` output) ahead of the re-arm, not
from the sixth.
