...
shape, fighter 1 has low halfword `+0x194 == 0` and fighter 0 has one of the
measured valid type-5 indices `+0x194 == {0x6f, 0x73, 0x74}`.
...
The native guard checks the measured low-halfword composition and admits only
those three indices. The nonzero `0x19ef8` selector, other type indices, and
sibling state/scaling compositions remain fail-closed.
