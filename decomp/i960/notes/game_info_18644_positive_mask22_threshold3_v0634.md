...
The no-bit-8 state-8 masks `0x00000022` (bits 1+5) and `0x00000024` (bits 2+5)
were measured across the three fighter-record distributions, countdown `0/1`
and mode bit 6 clear/set. Before admission, all 12 native runs for each mask differed from the oracle only by a
