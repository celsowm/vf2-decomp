# `fa_game_info` positive state-8 mask `0x1e` through threshold 9 (v0584)

The positive state-8 composition `0x0000001e` (bits 1+2+3+4) now uses the
existing measured mixed-low path through threshold `9`. The reference/native
full-dispatch threshold-9 slice covers the three measured fighter-record
...
python decomp/i960/tools/validate_game_info_full_dispatch.py \
  build/Debug/vf2i960.exe roms/vf2 --mask 0x1e \
