...
)
{
    ...
    if (counter0 == UINT32_C(1)) {
        ...
        cpu->registers[4] = 0u;
        set_equal_condition(cpu);
        status = vf2_model2a_write_u32(machine, VF2_TEXTURE_COUNTER0, 0u);
        if (status == VF2_OK) {
            status = vf2_model2a_read_u32(
                machine, VF2_TEXTURE_COUNTER0 + UINT32_C(4), &argument0
            );
        }
        if (status == VF2_OK) {
            status = vf2_model2a_read_u32(
                machine, VF2_TEXTURE_COUNTER0 + UINT32_C(8), &argument1
            );
        }
        if (status == VF2_OK) {
            status = vf2_model2a_read_u32(
                machine, VF2_TEXTURE_COUNTER0 + UINT32_C(12), &argument2
            );
        }
        if (status != VF2_OK) {
            return status;
    ...
    if (counter1 == UINT32_C(1)) {
        ...
        cpu->registers[4] = 0u;
        set_equal_condition(cpu);
        status = vf2_model2a_write_u32(machine, VF2_TEXTURE_COUNTER1, 0u);
        if (status == VF2_OK) {
            status = vf2_model2a_read_u32(
                machine, VF2_TEXTURE_COUNTER1 + UINT32_C(4), &argument0
            );
        }
        if (status == VF2_OK) {
            status = vf2_model2a_read_u32(
                machine, VF2_TEXTURE_COUNTER1 + UINT32_C(8), &argument1
            );
        }
        if (status == VF2_OK) {
            status = vf2_model2a_read_u32(
                machine, VF2_TEXTURE_COUNTER1 + UINT32_C(12), &argument2
            );
        }
        if (status != VF2_OK) {
            return status;
    ...
    if (counter2 == UINT32_C(1)) {
        ...
        if (argument0 > UINT32_C(0x56)) {
            uint64_t diagnostic_instructions = 0u;

            status = execute_texture_number_diagnostic(
                machine, cpu, argument0, &diagnostic_instructions,
                &changed_values, &bytes_written
            );
            if (status != VF2_OK) {
                return status;
            }
            ...
            cpu->executed_instructions +=
                UINT64_C(198) + (instructions - UINT64_C(12));

            report->kind = VF2_HYBRID_BRIDGE_TEXTURE_COUNTER_UPDATE;
            report->entry_address = VF2_TEXTURE_COUNTER_UPDATE_ENTRY;
            report->exit_address = VF2_TEXTURE_COUNTER_UPDATE_EXIT;
            report->iterations = UINT64_C(3);
            report->changed_values = UINT64_C(22) + changed_values;
            report->bytes_written = 46u + bytes_written;
            report->recovered_instruction_count =
                UINT64_C(198) + (instructions - UINT64_C(12));
            report->recovered_procedure_calls = UINT64_C(5) + procedure_calls;
            report->recovered_procedure_returns = UINT64_C(5) + procedure_returns;
            report->cpu_poststate_applied = 1;
            return VF2_OK;
        }
        status = vf2_model2a_write_u32(
